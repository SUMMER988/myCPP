...
string printSubstrings(const string& mainString, const string& lengthString) {
    ofstream outFile("0219_PrintSSS.txt");
    if (!outFile.is_open()) {
        cout << "无法打开文件！" << endl;
        return "";
    }
    ...
    ofstream concatenationFile("0219_PrintSSS_Concatenation.txt");
    if (!concatenationFile.is_open()) {
        cout << "无法打开文件！" << endl;
        outFile.close();
        return "";
    }
...
int main() {
    ...
    appToFile("0219_decodedString.txt", resultString + "\n");
